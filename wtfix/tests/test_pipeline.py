...
class TestBasePipeline:
    ...
    def test_get_app_chain_inbound(self, three_level_app_chain):
        pipeline = BasePipeline(installed_apps=three_level_app_chain)
        chain = list(pipeline.get_app_chain(pipeline.INBOUND))
        assert chain[0].name == "below"
        assert chain[1].name == "middle"
        assert chain[2].name == "top"

    def test_get_app_chain_outbount(self, three_level_app_chain):
        pipeline = BasePipeline(installed_apps=three_level_app_chain)
        chain = list(pipeline.get_app_chain(pipeline.OUTBOUND))
        assert chain[0].name == "top"
        assert chain[1].name == "middle"
        assert chain[2].name == "below"

    def test_get_app_chain_direction_unknown(self, three_level_app_chain):
        with pytest.raises(ValidationError):
            pipeline = BasePipeline(installed_apps=three_level_app_chain)
            pipeline.get_app_chain("inbound")
    ...
    def test_receive(self, three_level_app_chain):
        ...
        assert pipeline.receive("Test") == "Test r1 r2 r3"
